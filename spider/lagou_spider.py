...
class LagouSpider(object):
    ...
    def init_logger(self):
        ...
        ch_formatter = logging.Formatter("%(asctime)s - %(name)s - %(funcName)s - %(lineno)d - %(levelname)s - %(message)s")
            ...
            for label_index in xrange(1, city_label_num+1):
                ...
                for i in xrange(1, city_num+1):
                    city_element = self.driver.find_element_by_xpath(
                        '//*[@id="main_container"]/div/div[1]/table[2]/tr[{0}]/td[2]/ul/li[{1}]'.format(label_index, i))
    ...
    def get_postions(self, kd, first, pn, city):
        ...
        url = 'https://www.lagou.com/jobs/positionAjax.json?px=default&{0}&needAddtionalResult=false'.format(city_encoded)
        data = {'kd': kd, 'first': first, 'pn': pn}
